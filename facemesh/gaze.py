...
output_video_name = 'gaze_mesh.avi'
output_landmark_file_name = 'gaze_landmarks.csv'
        ...
        while cap.isOpened():
            ...
            if results.multi_face_landmarks:
                for face_landmarks in results.multi_face_landmarks:
                    for id in IRIS_LANDMARKS:
                        lm = face_landmarks.landmark[id]
                        writer.writerow({'frame_num': frame_num, 'landmark_x': lm.x, 'landmark_y': lm.y})

                    # Draw only mouth landmarks on video output
                    mouth_landmarks = landmark_pb2.NormalizedLandmarkList()
                    mouth_landmarks.landmark.extend([face_landmarks.landmark[id] for id in IRIS_LANDMARKS])
